...
class MistralOptimizationPatcher(Patcher):
    ...
    def patch(self, target: MistralPreTrainedModel):
        ...
        for n, m in target.named_modules():
            if self.fused_cross_entropy and isinstance(m, MistralForCausalLM):
                self.patch_method(m.forward, _mistral_for_causal_lm_forward)
            
            if self.fuesed_rms_norm and isinstance(m, MistralRMSNorm):
                self.patch_method(m.forward, _mistral_rms_norm_forward)

            if self.fused_rotary_embedding:
                ATTENTION_FORWARD_FN_MAPPING = {
                    MistralAttention: _mistral_attention_forward,
                    MistralSdpaAttention: _mistral_sdpa_attention_forward,
                    MistralFlashAttention2: _mistral_flash_attention_2_forward
                }
            ...
            if self.fused_swiglu and isinstance(m, MistralMLP):
                self.patch_method(m.forward, _mistral_mlp_forward)
...
def _mistral_for_causal_lm_forward(
    ...
    **kwargs
    ...
    outputs = MistralForCausalLM.forward(self, *args, return_dict=return_dict, **kwargs)
...
def _mistral_rms_norm_forward(self: MistralRMSNorm, x: torch.Tensor):
...
def _set_cos_sin_cache(self: MistralRotaryEmbedding, seq_len: int, device: torch.device, dtype: torch.dtype):
...
def _get_rotary_embedding(rotary_emb: MistralRotaryEmbedding):
...
def _mistral_attention_forward(
...
def _mistral_sdpa_attention_forward(
    self: MistralSdpaAttention,
    hidden_states: torch.Tensor,
    ...
    if output_attentions:
        ...
        return super(MistralSdpaAttention, self).forward(
...
def _mistral_flash_attention_2_forward(
...
def _mistral_mlp_forward(self: MistralMLP, x: torch.Tensor):
